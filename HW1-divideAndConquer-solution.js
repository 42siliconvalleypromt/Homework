...
function numberOfOnes(arr) {
  ...
  while(start < end) {  
    let midpoint = Math.floor(start+Math.floor(end-start)/2)
...
function closestValue(arr, target) {
  ...
  while(start < end) {
    midpoint = Math.floor(start+(Math.floor(end-start)/2))

    if(Math.abs(value - target) >Math.abs(arr[midpoint]-target) ) {
      value = arr[midpoint]
